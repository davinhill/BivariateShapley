...
from model import NN, COPD

# Set path for model and data locations
path_data = '../Data/COPD.h5'
...
train_set = COPD(data_path = path_data, train = True)
train_loader = torch.utils.data.DataLoader(
    train_set, batch_size=len(train_set), shuffle=True, num_workers=0)

test_set = COPD(data_path = path_data, train = False)
...
for epoch in range(init_epoch, num_epochs):
    ...
    if epoch % 10 == 0: print("Epoch: %d || Loss: %f || Train_Accy: %f || Test_Accy: %f || Time: %s" %
...
torch.save(model, os.path.join(path_model, 'COPD_model.pt'))
